/// AES (Advanced Encryption Standard) implementation module
/// Contains the S-box lookup table and encryption functions used for side-channel analysis

/// Standard AES S-box lookup table
/// Used for SubBytes transformation in AES encryption
const SBOX: [u8; 256] = [
...
/// Performs AES S-box lookup after XOR operation
///
/// # Arguments
/// * `p` - Single byte plaintext
/// * `k` - Single byte key
///
/// # Returns
/// The S-box substitution result after XORing plaintext with key
pub fn aes_8(p: &u8, k: &u8) -> u8 {
    SBOX[(p ^ k) as usize]
}
